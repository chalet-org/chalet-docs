...
In order to receive information about a project, Chalet can be queried to either get a list of something or some JSON back, depending on the query. This is done with the `query` subcommand. 
...
In the case of build file state changes, it could mean build the name of a build target has changed, or a build configuration was changed. Here's an example result from a project called `foo`:
