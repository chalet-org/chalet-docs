...
Any edits done in the `chalet.json` file will require the .sln to be re-exported.
...
This will generate the same project files as the export method above, and build them via [MSBuild](https://docs.microsoft.com/en-us/visualstudio/msbuild/msbuild?view=vs-2022), using the same build cache that Visual Studio uses, so you use the project within VS and the command line interchangeably.
...
The files are generated to `build/.vsjson` as not clash with any existing files that may be checked-in. You would then copy those files to `(project root)/.vs`.
...
The executable projects should be selectable in this dropdown:
