...
[Xcode](//developer.apple.com/xcode) is Apple's IDE, designed to develop software for its platforms. You can get it from the [Apple Developer downloads](//developer.apple.com/download/all/) page or through the app store. Generating an Xcode project from Chalet is pretty straightforward.

\\

## [Export a project]

The following will generate an Xcode project (.xcodeproj) into `build/.xcode`. It's designed to open and build to the same directories that Chalet normally uses, although this is still somewhat a work in-progress since Xcode generates some extra directories.

```bash
chalet export -t apple-llvm xcode
```

To open it, this requires an Xcode install. You cannot use Command Line Tools with this. Any edits done in the [build file](/docs/the-build-file) will require the Xcode project to be re-exported.

Once in Xcode, each build target will be selectable in the [project's toolbar](//developer.apple.com/documentation/xcode/customizing-the-build-schemes-for-a-project).
...
More information can be found here: [Customizing the build schemes for a project](//developer.apple.com/documentation/xcode/customizing-the-build-schemes-for-a-project)
...
The output is cleaned up with `--no-show-commands` (default), so if you need to see the full xcode build output, use `--show-commands`.
