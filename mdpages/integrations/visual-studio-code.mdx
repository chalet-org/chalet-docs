...
| Toolchain or Preset | The toolchain or preset, as defined by either the project's `.chaletrc` file, or in `.chaletconfig` |
...
| Command | The last button will simply run the Chalet command |

When a command is run, the extension opens a pseudo-terminal and runs Chalet via a background process, so it has its own "Chalet" terminal instance (instead of bash, powershell, VS Code task, etc).
...
Firstly, as of version 1.66, VS Code uses a higher default contrast ratio to adjust terminal colors to a more accessible color-range. This may be confusing if the colors you're expecting from your theme or settings.json are not showing up correctly. This may affect the Chalet theme being used. To adjust this, change the `Terminal › Integrated: Minimum Contrast Ratio` setting to `1`. If using the JSON settings, it's under `terminal.integrated.minimumContrastRatio`:

This simply restores the old (< v1.66) default.

**Windows 10**

Another odd terminal problem to address is that on Windows, ANSI terminal styles are controlled by the console host windows process (`conhost.exe`). VS Code uses the OS' one to render styles in the terminal. On Windows 10 specifically (as of version 10.0.19041.x), that process does not render all ANSI styles correctly. It may get addressed on Windows 10 in a future update, but Windows 11 does not have this problem.
