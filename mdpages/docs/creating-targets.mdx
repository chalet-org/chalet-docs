...
| `${externalDir:(Name)}` | The path to an external dependency by name. See [externalDependencies](/schema/latest/chalet-json?id=external-dependencies) |
| `${externalBuildDir:(Name)}` | The build path for an external dependency by name. For instance, if a CMake project is located at `${externalDir:foo}`, it will build to `${buildDir}/${externalDir}/foo`, so it's easier to write `${externalBuildDir:foo}` |
