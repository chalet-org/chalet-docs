...
| `${so:(Name)}` | Outputs a shared library filename appropriate to the platform, toolchain & architecture with `name` as the basename |
...
| Windows - MSVC | math.dll | glm.lib | prog.exe |
