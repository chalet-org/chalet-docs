...
If a project has more than one runnable target, it may be wanted to set the default run target explicitly. This can be achieved by setting the `defaultRunTarget` property in the root. For example:
...
```json
{
   ...
   "defaultRunTarget": "second-executable"
}
```

The run target can be selected at the command line during `run` and `buildrun` commands, but this will set the target initially if one is omitted otherwise.
