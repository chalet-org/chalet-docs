...
| `${externalBuild:(Name)}` | The build path for an external dependency by name. For instance, if a CMake project is located at `${external:foo}`, it will build to `${buildDir}/${externalDir}/foo`, so it's easier to write `${externalBuild:foo}` |
