...
Validation targets are an easy way to validate a JSON file or group of JSON files against a (local) JSON schema of your choice. This uses the same mechanism that Chalet uses under the hook to validate `chalet.json` and `.chaletrc` against their respective schemas at build time. To create a validation target, set `kind` to `validation`, define the `schema` to use (only [Draft-07](//json-schema.org/draft-07/json-schema-release-notes) is supported), and add any files you wish to validate. Files can take a relative path of files, or a glob pattern.
...
| `${link:(Name)}` | Outputs a shared library filename appropriate to the platform, toolchain & architecture with `name` as the basename |
| `${lib:(Name)}` | Outputs an lib/archive (static library) filename appropriate to the platform, toolchain & architecture with `name` as the basename |
| `${exec:(Name)}` | Outputs an executable filename appropriate to the platform with `name` as the basename |
...
| Variable | `${link:math}` | `${lib:math}` | `${exec:prog}`  |
| Windows - MSVC | math.dll | glm.lib | game.prog |
...
`lib` is not `staticLink` because it can either be a static library or an archive built alongside a .dll (as is with MinGW)
