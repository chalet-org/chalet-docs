...
Build a project and create its configuration if it doesn't exist.

```bash
chalet rebuild [options] [<target>]
```

Rebuild the project and create a configuration if it doesn't exist.

```bash
chalet clean [options]
```

Removes the build folder for the current build configuration.

```bash
chalet bundle [options]
```

Bundle a project for distribution with the current build configuration.
...
| vs-preview | Microsoft® Visual Studio (latest installed preview/insider release) |
| vs-2026 | Microsoft® Visual Studio 2026 (Insiders) |
...
| llvm-vs-preview | LLVM/Clang in Microsoft® Visual Studio (latest preview/insiders) |
| llvm-vs-2026 | LLVM/Clang in Microsoft® Visual Studio 2026 |
...
<dd>
An optional config to give to the profiler during profiling.
