...
Use the global settings: `~/.chalet/config`
...
The project format kind to export. (experimental)
...
| schema-settings-json | The entire `.chaletrc` / `.chalet/config` schema in JSON format |
...
| state-settings-json | State applicable to the `.chaletrc` / `.chalet/config` for the current project in JSON format |
