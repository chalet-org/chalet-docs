...
| seachPaths | Additional relative paths to search for compiled dependencies not otherwise picked up. |
...
The directory can be referenced in chalet.json with the `${externalDir}` path substitution variable explained [in the next section](/docs/creating-targets?id=path-substitution).
