...
> Note: Transitive dependencies aren't handled yet, so if one package has an include for a version of a header-only library, and another package has another version of the same library, both will be imported. In the future, `package` itself may have its own `importedPackages`
