...
1. Use the following. The `latest` part of the url can be replaced with the version (ie. `v0.6.6`)

<Code lang="bash" copyButton>
brew install --cask https://www.chalet-work.space/api/brew/latest/chalet.rb
</Code>

2. The shell completion scripts will be in `/opt/homebrew/Caskroom/chalet/(version)`
...
You'll be shown a wizard that will guide you the rest of the way, creating a simple "Hello World" project, a [build file](/docs/the-build-file), and optionally a git repository. In the last step, you'll be asked to configure the project.
