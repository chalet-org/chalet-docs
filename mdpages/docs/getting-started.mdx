...
1. Use the following. The `latest` part of the url can be replaced with the version (ie. `v0.6.5` or later)
...
2. Extract to `/opt/chalet` and add it to `PATH` (optionally, you can define `CHALET_PATH` and add that to `PATH`)
