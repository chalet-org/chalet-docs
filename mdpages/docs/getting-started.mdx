...
chalet configure -t vs-2026 -a arm64
...
chalet configure -t vs-2026 -a arm64 -b msbuild
...
To remove the project's build files at any point for the configured build, use the following:

```bash
chalet clean
```

To remove all of the project's generated files, add the `--all` options:
