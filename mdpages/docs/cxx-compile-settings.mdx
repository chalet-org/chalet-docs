...
The only Chalet-specific caveat is that a PCH can't be placed at the root directory (for instance, where the `chalet.json` file is). It must be placed inside of a subdirectory like `src`. This is to keep build behavior consistent across platforms.
