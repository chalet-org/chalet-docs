...
* [Script Target](/schema/latest/cmake-target) - A path to an individual script
...
Script targets are quite simple. In addition to the `condition` and `description` properties, they only take a `script` property (that can also be suffixed with a property condition.
