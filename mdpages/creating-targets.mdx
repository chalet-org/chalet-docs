...
Most properties support rules that describe whether to include or exclude them from the build. These are simply suffixes that describe the build configuration or platform:
...
Most properties support both, but some support one and not the other where appropriate.

When combining the two, order as: `:debug.(platform)`.

If a property with these rules are defined along with a property without them (for example: `links`, `links:debug.windows` & `links.windows`), `links` will be read first, `links.windows` second, followed by `links:debug.windows`.
...
Finally, one can also include/exclude based on platform using the property rules explained earlier:
...
Property rules are also supported:
