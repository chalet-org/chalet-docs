...
Some of these properties will be covered here, while others will be explained are detailed in the schema documentation, [here](http://localhost:3000/schema-dev/latest/chalet-json/executable-source-target).
...
* `${externalDir}` - resolves to the root path for external dependencies. See [externalDependencies](/schema-dev/latest/chalet-json?id=external-dependencies)
