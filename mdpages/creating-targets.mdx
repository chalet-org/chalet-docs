...
* [Source Target](/schema/latest/executable-source-target) - A folder, set of folders, or files with compilation options
...
* [Script Target](/schema/latest/build-script-target) - A path to an individual script
...
* **strict** & **strictPedantic** - Equivalent to `/W4 /WX`. Warnings will be treated as errors.
* **veryStrict** - Equivalent to `/Wall /WX`. Warnings will be treated as errors.
...
As a last resort, `compileOptions` and `linkerOptions` can pass certain options directly to the build at the time of a compilation unit's compile, or at link time. However, this ultimately limits which compilers can be used with your project, because property conditions aren't specific enough yet to target compilers and versions. As Chalet evolves, more options will be leveraged as properties, and the goal will be to reduce the need for this as much as possible, or also provide some kind of lightweight scripting. If you require a more refined build, you can certainly combine CMake with Chalet.
...
Chalet targets are like saying "I have a Chalet project in a separate repository and want it to build during this project's build."  This is a feature that may not get a lot of use in these early days, but could serve as a foundation for package management in the future.
