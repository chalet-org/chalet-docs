...
* For targeting compilers and architectures, see [Advanced: Fine Tuning](/cxx-compile-settings?id=advanced-fine-tuning)
...
* `${externalDir}` - resolves to the root path for external dependencies. See [externalDependencies](/schema/latest/chalet-json?id=external-dependencies)
...
<!-- nav:/managing-dependencies:/cxx-compile-settings -->
