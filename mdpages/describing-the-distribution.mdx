...
* [Bundle](/schema/latest/dist-bundle) - A group of files/folders, optionally defined by a subdirectory. On MacOS, this can be an app bundle
* [Archive](/schema/latest/dist-archive) - Describes files/folders to include in a .zip archive
* [Script](/schema/latest/dist-script) - A path to an individual script to run
* [MacOS Disk Image](/schema/latest/dist-macos-disk-image) - Describes a MacOS DMG disk image to create
* [Windows Nullsoft Installer](/schema/latest/dist-windows-nullsoft-installer) - Describes a Nullsoft installer to create for Windows applications
...
* `${name}`: The bundle's name, so in the examples above, this would be "MyApp"
* `${bundleName}`: The first 15 characters of ${name}, specifically used for [CFBundleName](https://developer.apple.com/documentation/bundleresources/information_property_list/cfbundlename)
* `${mainExecutable}`: The main executable, explained [here](/describing-the-distribution?id=the-main-executable)
...
<!-- nav:/creating-targets:/command-usage -->
