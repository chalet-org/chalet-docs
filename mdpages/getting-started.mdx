...
```bash
chalet configure --toolchain msvcpre
```
...
The toolchain refers to either a preset (`llvm` `applellvm` `gcc` `msvc` `msvcpre`) or a custom name that resolves to a toolchain structure in the project settings. The preset will attempt to detect compiler toolchain executables add add them to settings. 
...
```bash
chalet configure -t msvc -a x64 
chalet configure -t msvcpre -a x64
# Attempts to find the target architecture and creates the toolchain 'x64-pc-windows-msvc(version)' 
# where 'version' is the detected MSVC version (or Preview version)

chalet configure -t gcc -a x64 
# Attempts to find the target architecture (if MinGW) and creates the toolchain 'x64-gcc'
# since the version of GCC (and the target triple) do not get inferred (yet)

chalet configure -t llvm -a x64
chalet configure -t applellvm -a x64 
# creates the toolchain 'llvm' or 'applellvm'
# since Clang handles the architecture
```

> Note: In all cases, you can edit the generated toolchain name later

> Note: The output color scheme can also be controlled via `.chaletrc`, but does not get copied from the global settings
...
If the output directory is `build`, this will generate a build of the project into the folder `build/(toolchain)_(configuration)`. The subfolder can be customized in the toolchain settings.

By default, Chalet has 5 build configurations: `Release` `Debug` `MinSizeRel` `RelWithDebInfo` and `Profile`. It uses the same build configuration names as CMake, specifically so that it can be passed easily to CMake build targets. `Profile` is a special target that makes a build for profiling and attempts to launch a compatible profiling utility. In the Profile build of CMake build targets, `Debug` is used.

Any Chalet command that takes a build configuration is related to that specific build & configuration, such as:

```bash
chalet buildrun Debug
chalet run Debug
chalet build Debug
chalet rebuild Debug
chalet clean Debug
```

The `buildrun` & `run` commands can also take the target you want to run afterwards (including its arguments). Therefor, if multiple executables are built, and one wants to run a target called "my-tests", a project that is not the default run project, one can use:
```bash
chalet buildrun Debug my-tests
``` 
