...
The toolchain refers to either a preset (`llvm` `apple-llvm` `gcc` `vs-stable` `vs-preview`) or a custom name that resolves to a toolchain structure in the project settings file. A preset will attempt to detect compiler toolchain executables add add them to the settings file. 
...
# 'x64-pc-windows-msvc(version)' where 'version' is the one detected 
# VS version (or Preview version) - 16, 17, etc.
...
# Creates the toolchain 'llvm' or 'apple-llvm' since Clang handles the architecture
...
# "auto" explicity sets the output architecture to the host machine's architecture
chalet configure -t llvm -a auto

# Creates a generic GCC toolchain and attempts to find the version with the target architecture
chalet configure -t gcc -a auto 

# In most cases, just the toolchain with the full triple will suffice
chalet configure -t x86_64-linux-gnu-gcc 
chalet configure -t x86_64-x64-mingw32-gcc 
chalet configure -t x86_64-apple-darwin20-gcc-11 
...
The first four use the same names as CMake, specifically for familiarity. `Profile` is a special target that makes a build for profiling and attempts to launch a compatible profiler. In the Profile build of CMake build targets, `Debug` is used, while if custom build configurations are defined, a CMake compatible configuration will be deduced from its properties. For those familiar with optimization levels, `Release` uses `-O3` with link-time optimizations, while `RelStable` uses `-O2` and no link-time optimizations. `RelStable` is provided for builds where `-O3` and `-lto` do not make sense.
