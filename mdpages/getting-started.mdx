...
The `buildrun` & `run` commands can also take the target you want to run afterwards (including its arguments). Therefore, if multiple executables are built, and one wants to run a target called "my-tests", a project that is not the default run project, one can use:
```bash
