...
```bash
# To target a non-specific stable/preview version:
chalet configure --toolchain vs-stable
chalet configure --toolchain vs-preview 

# To target specific VS versions:
...
The toolchain refers to either a preset (`llvm` `apple-llvm` `gcc` `vs-stable` `vs-preview`) or a custom name that resolves to a toolchain structure in the project settings. The preset will attempt to detect compiler toolchain executables add add them to settings. 
...
```bash
chalet configure -t vs-stable -a x86 
